...
DenoisePass::DenoisePass(std::string_view name) noexcept
    ...
    if (!s_enabled_flag) {
        EventDispatcher<ECanvasEvent::Display>(wavefront::PTPass::PT_RESULT_BUFFER);
    }
}
...
void DenoisePass::SetScene(World *world) noexcept {
    ...
    m_data.albedo = buf_mngr->GetBuffer("albedo buffer")->cuda_ptr;
    m_data.normal = buf_mngr->GetBuffer("normal buffer")->cuda_ptr;
