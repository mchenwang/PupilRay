...
extern "C" __global__ void __raygen__main() {
    ...
    if (index >= optix_launch_params.path_record.GetNum()) return;
    auto ray_dir    = optix_launch_params.path_record[index].ray_dir();
    auto ray_origin = optix_launch_params.path_record[index].ray_origin();
...
extern "C" __global__ void __closesthit__default() {
    ...
    auto               ray_dir     = optix_launch_params.path_record[index].ray_dir();
    auto               ray_origin  = optix_launch_params.path_record[index].ray_origin();
    auto               throughput  = optix_launch_params.path_record[index].throughput();
    auto               random_seed = optix_launch_params.path_record[index].random_seed();
    auto               pixel_index = optix_launch_params.path_record[index].pixel_index();
    ...
    auto hit = optix_launch_params.hit_record.Alloc();
    hit.mat(hit_info.mat);
    hit.geo(hit_info.geo);
    hit.ray_dir(ray_dir);
    hit.throughput(throughput);
    hit.emitter_index(sbt_data->emitter_index);
    hit.random_seed(random_seed);
    hit.pixel_index(pixel_index);

    if (optix_launch_params.depth == 0) {
        if (sbt_data->emitter_index >= 0) {
            auto& emitter                                 = optix_launch_params.emitters[sbt_data->emitter_index];
            optix_launch_params.frame_buffer[pixel_index] = make_float4(emitter.GetRadiance(hit_info.geo.texcoord), 1.f);
        }
    ...
    auto bsdf_sample_type = optix_launch_params.path_record[index].bsdf_sample_type();
    auto bsdf_sample_pdf  = optix_launch_params.path_record[index].bsdf_sample_pdf();
